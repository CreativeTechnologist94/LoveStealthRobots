...
public class ViewCulling : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        {
            Debug.Log(enemy.name + " is out of view now");
        ...
        {
            Debug.Log(enemy.name + " is in view now");
            var rend = enemy.GetComponentInChildren<SkinnedMeshRenderer>();
            if (rend)
            {
                rend.enabled = true;
        ...
        _enemiesInViewLastFrame = new List<Transform>(_fov.visibleObjects);                                              //copy and not just refer
    }
}
